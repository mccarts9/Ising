# -*- coding: utf-8 -*-
"""
This is an investigation of run time
Make an array of system sizes, and plot against time to equilibrise each of them
...
plt.figure(1)
plt.scatter(amount, times)
plt.xlabel('N = Length of one row')
plt.ylabel('Time for Ising Model to have equal spin states.')
...
coef = np.lib.polyfit(log_sizes, log_times, 1) 
...
plt.figure(2)
plt.plot(log_sizes, log_times, 'ro', label = 'Log of time duration of Ising Model')
plt.plot(log_sizes, line, 'b--', label = log_label)
plt.legend(loc=2)
plt.xlabel('Log of Length of one row')
plt.ylabel('Log of time duration of Ising Model')
