...
This is an investigation of system size
It plots temperature against the system energy after N=3.5 iterations for a few different sample sizes
...
#Parameters
N = [1, 2, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50]     #matrix size N rows and N columns
...
for i in N:
    ...
    for elem in temps:
        matr = initialise(i)
        matrp = Ising(i, matr, elem)
        sys_energy = total_energy(matrp, i)
        energies.append(sys_energy)    
    plt.figure()
    plt.scatter(temps, energies)
    title = 'System Energy after N**3.5 iterations, where N = ' + str(i)
    plt.xlabel('Temperature')
    plt.ylabel('System Energy')
    plt.title(title)

    prog +=1
    print('I have ', prog, 'temperatures completed out of ', len(N))
