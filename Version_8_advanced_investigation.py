...
This one plots the system energy after N**3.5 iterations at T = 1 against a range of system sizes
"""
...
#Parameters
amount = np.linspace(2, 14, 7)    #matrix size N rows and N columns
...
for i in amount:
    ...
    sys_energy = total_energy(matrp, i)
...
plt.figure(1)
plt.scatter(amount, energies)
plt.xlabel('N = Length of one row')
plt.ylabel('System Energy after N**3.5 iterations at T = 1')
plt.title('System Size and System Energy after N**3.5 iterations at T = 1')

log_sizes = np.log10(amount)
log_energies = np.log10(np.abs(energies))

coef = np.lib.polyfit(log_sizes, log_energies, 1) 
...
plt.figure(2)
plt.plot(log_sizes, log_energies, 'ro', label = 'Log of absolute value of system energy')
plt.plot(log_sizes, line, 'b--', label = log_label)
plt.legend(loc=2)
plt.xlabel('Log of Length of one row')
plt.ylabel('Log of Absolute Value of System Energy ')
plt.title('Log Log Plot of System Size and Absolute Value of System Energy after N**3.5 iterations at T = 1')
plt.show()